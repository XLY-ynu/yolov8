...
listdir = np.array([i for i in os.listdir(xmlpath) if i.endswith('.xml')])
...
for i in train:
    ...
    shutil.copy(f'{xmlpath}/{i}', f'labels/train/{i}')

for i in val:
    shutil.copy(f'{imgpath}/{i[:-4]}.{postfix}', f'images/val/{i[:-4]}.{postfix}')
    shutil.copy(f'{xmlpath}/{i}', f'labels/val/{i}')

for i in test:
    shutil.copy(f'{imgpath}/{i[:-4]}.{postfix}', f'images/test/{i[:-4]}.{postfix}')
    shutil.copy(f'{xmlpath}/{i}', f'labels/test/{i}')
